...
else
{
	...
	$results = Db::getInstance()->ExecuteS("SELECT uc.*,o.id_order FROM "._DB_PREFIX_."jostpay uc JOIN "._DB_PREFIX_."orders o ON uc.cart_id=o.id_cart  WHERE uc.transaction_id='$transactionId' LIMIT 1");
	...
	else
	{
		...
		$url="https://jostpay.com/api_v1?ref=$transactionId&action=get_transaction&jostpay_id=$cipgid&amount=$total";
		$ch = curl_init();
