<?php
	...
	$page_url=(Configuration::get('PS_SSL_ENABLED') ? 'https://' : 'http://').htmlspecialchars($_SERVER['HTTP_HOST'], ENT_COMPAT, 'UTF-8').__PS_BASE_URI__.'modules/jostpay/jostpay_transactions.php';
		...
		$results = Db::getInstance()->ExecuteS("SHOW TABLES LIKE '"._DB_PREFIX_."jostpay'");
		...
		else
		{
			if(!empty($_REQUEST['transaction_id']))
			{
				...
				$success=false;
				$results = Db::getInstance()->ExecuteS("SELECT uc.*,o.id_order FROM "._DB_PREFIX_."jostpay uc JOIN "._DB_PREFIX_."orders o ON uc.cart_id=o.id_cart  WHERE uc.transaction_id='$transaction_id' LIMIT 1");
				...
				else
				{
					...
					$cipgid= Configuration::get('JOSTPAY_MERCHANT_ID');
					$total=$row['transaction_amount'];
		
					$url="https://jostpay.com/api_v1?ref=$transaction_id&action=get_transaction&jostpay_id=$cipgid&amount=$total";
					...
					elseif(!empty($json))
					{
						...
						Db::getInstance()->execute("UPDATE "._DB_PREFIX_."jostpay SET
			...
			if($access_type=='admin')$sql="SELECT COUNT(*) FROM "._DB_PREFIX_."jostpay ";
			else $sql="SELECT COUNT(*) FROM "._DB_PREFIX_."jostpay  WHERE customer_id='".$cookie->id_customer."'";
			
			if( Db::getInstance()->getValue($sql)==0)$toecho.="<h3>No record found for transactions made through JostPay</h3>";
			else
			{
			...
			if($access_type=='admin')$sql="SELECT * FROM "._DB_PREFIX_."jostpay ";
			else $sql="SELECT * FROM "._DB_PREFIX_."jostpay  WHERE customer_id='".$cookie->id_customer."'";
